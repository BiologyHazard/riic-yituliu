        ...
        <div class="schedule-container">
            <div class="schedule-line" v-for="(stationLine, lineIndex) in props.lines" :key="lineIndex">
                <div class="queue-descriptions">
                    <div class="queue-description" v-for="(description, index) in props.queueDescription" :key="index">
                        {{ `队列 ${index + 1}` }}<br>{{ description }}
                    </div>
                </div>
                <div class="stations">
                    <Station v-for="(station, stationIndex) in stationLine" :key="stationIndex" v-bind="station" />
                </div>
            </div>
        </div>
    </div>
