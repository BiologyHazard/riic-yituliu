...
const charName = ref('阿米娅')
const eliteLevel = ref('0')
const rarity = ref('5')
const profession = ref('术师')
...
<template>
  <div>
    ...
    <div class="input-group">
      ...
      <input id="operatorName" v-model="charName" type="text" />
    </div>
    <div class="input-group">
  ...
  <div>
    <OperatorCard :is-tired="isTired" :char-id="charId || (getCharIdbyName(charName) ?? '')" :displayName="charName"
      :eliteLevel="eliteLevel ? Number(eliteLevel) : null" />
  </div>
  <div>
    <OperatorAvatar style="width: 240px; height: 240px; border-radius: 120px"
      :char-id="charId || (getCharIdbyName(charName) ?? '')" :elite-level="eliteLevel ? Number(eliteLevel) : undefined"
      :is-tired="isTired" :profession="profession || undefined" :rarity="rarity ? Number(rarity) : undefined"
      :show-background-image="showBackgroundImage" :show-rarity="showRarity" :show-profession="showProfession"
      :show-elite-level="showEliteLevel" />
  </div>
  {{ { charId, charName, eliteLevel, rarity, isTired } }}
</template>
